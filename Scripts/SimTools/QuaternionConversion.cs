...
public class QuaternionConversion
{
    ...
    #region Alex's Attempt to Emulate Heading, Bank Angle, and Angle of Attack

    //first we need to unwind the pitch value (where pitch is represented as heading)
    /// <summary>
    /// Calculates the Yaw (or more precisely the "heading" of the spaceship relative to the origin).
    /// </summary>
    /// <param name="player">The player spaceship transform</param>
    /// <returns>a float value from 0 - 360 using left-hand-rule rotation </returns>
    /// <remarks>Math formulas derived by Alex Redei on 5/17/20 using truth tables</remarks>
    public float getYaw(Transform player)
    {
        return 0.0f;

        /*
        //float angle = Vector3.Angle(player.forward.z, player.forward.x);

        double x = Math.Round(player.transform.forward.x, 2);
        double z = Math.Round(player.transform.forward.z, 2);
        double rawAngle = (Math.Asin(x) * (180 / System.Math.PI));
        float angle = (float)Math.Round(rawAngle, 2);

        //conver the angle to a positive number if negative
        ...
        {
            angle += 180;
            //add 180 to the roll angle
            //add the component that is moving in the backwards direction to the angle.
        ...
        //Vector3 v = player.InverseTransformPoint(player.forward);
        //float x = v.x;
        //float y = v.y;
        //float z = v.z;
        //return x;   //the component moving along the x axis
        //return Vector3.SignedAngle(player.forward, Vector3.forward, Vector3.up);
        //return Vector3.Angle(Vector3.forward, player.forward);

        //note this is -1 to + 1 value

        //if 
        //float angle = Vector3.Angle(Vector3.forward, player.forward);
        //return angle;
        */
    }
    ...
    public float getPitch(Transform player)
    {
        ...
        //return Vector3.SignedAngle(Vector3.up, player.up, Vector3.right);

    }
    public float getRoll(Transform player)
    {
        double x = Math.Round(player.transform.up.x, 2);
        double y = Math.Round(player.transform.up.y, 2);
        double rawAngle = (Math.Asin(x) * (180 / System.Math.PI));
        float angle = -(float)Math.Round(rawAngle, 2);
    ...
    #region Keep Track of Prior Inspector Value Rotations Over Time - Need a way to unwind the yaw value maybe (using code already established above)?
    //To BE DONE
